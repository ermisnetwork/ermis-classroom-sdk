...
export async function initAudioRecorder(audioStream, options = {}, existingAudioContext = null) {
  ...
  try {
    console.log("[initAudioRecorder] Using provided MediaStreamTrack");

    let context;
    if (existingAudioContext) {
      console.log("[initAudioRecorder] Reusing existing AudioContext, state:", existingAudioContext.state);
      context = existingAudioContext;
    } else {
      const AudioContext = window.AudioContext || window.webkitAudioContext;
      console.log("[initAudioRecorder] Creating new AudioContext...");
      context = new AudioContext({
        sampleRate: finalOptions.encoderSampleRate,
      });
      console.log("[initAudioRecorder] AudioContext created, state:", context.state);

      if (context.state === "suspended") {
        console.log("[initAudioRecorder] Attempting to resume AudioContext...");
        try {
          await context.resume();
          console.log("[initAudioRecorder] AudioContext resumed, state:", context.state);
        } catch (resumeErr) {
          console.error("[initAudioRecorder] Resume failed:", resumeErr);
        }
      }
    }

    console.log("[initAudioRecorder] Creating MediaStreamSource...");
    const sourceNode = context.createMediaStreamSource(audioStream);
    console.log("[initAudioRecorder] MediaStreamSource created");
    ...
    console.log("[initAudioRecorder] Recorder options:", recorderOptions);

    console.log("[initAudioRecorder] Creating Recorder instance...");
    const recorder = new Recorder(recorderOptions);
    console.log("[initAudioRecorder] Recorder created, waiting for initialize...");
    
    // Chờ Recorder initialize với timeout
    if (recorder.initialize) {
      try {
        const initTimeout = new Promise((_, reject) => 
          setTimeout(() => reject(new Error("Recorder init timeout")), 5000)
        );
        await Promise.race([recorder.initialize, initTimeout]);
        console.log("[initAudioRecorder] Recorder initialized successfully");
      } catch (initErr) {
        console.error("[initAudioRecorder] Recorder init error/timeout:", initErr);
        // Try to continue anyway with ScriptProcessor fallback might have been used
      }
    }
    ...
    console.error("[initAudioRecorder] Error initializing recorder:", err);
    throw err;
  }
}
