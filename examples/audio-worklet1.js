class JitterResistantProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.port.onmessage = (event) => {
      const { type, channelData, sampleRate, numberOfChannels, port } =
        event.data;
      if (type === "connectWorker") {
        ...
        if (
          ...
          this.sampleRate = sampleRate;
          this.numberOfChannels = numberOfChannels;
          this.fadeInLength = Math.round(sampleRate / 100); // 10ms fade-in
          // Resize buffer arrays if channel count changed
          this.resizeBuffers(numberOfChannels);
          console.log(
            `Processor configured: ${sampleRate}Hz, ${numberOfChannels} channels.`
          );
        }
        console.log("Received audio data in AudioWorklet:", channelData);
        // The 'channelData' is expected to be an Array<Float32Array>
        this.addAudioData(channelData);
      }
    };
  }
