...
export class Participant extends EventEmitter {
  ...
  constructor(config: ParticipantConfig) {
    ...
    this.permissions = config.permissions || {
      ...
      can_publish_sources: [["mic_48k", true], ["cam_360p", true], ["cam_720p", true], ["cam_1080p", true], ["cam_1440p", true], ["screen_share_720p", true], ["screen_share_1080p", true], ["screen_share_audio", true]],
      hidden: false,
  ...
  togglePin(): void {
    if (!this.isLocal) {
      if (this.isPinned) {
        this.subscriber?.switchBitrate("cam_360p");
        console.warn("Unpin participant, switch to low quality");
      } else {
        this.subscriber?.switchBitrate("cam_720p");
  ...
  get isCameraBanned(): boolean {
    if (!this.permissions.can_publish_sources) return false;
    for (const [channel, allowed] of this.permissions.can_publish_sources) {
      if ((channel === "cam_360p" || channel === "cam_720p" || channel === "cam_1080p") && allowed === false) {
        return true;
      }
    }
    return false;
