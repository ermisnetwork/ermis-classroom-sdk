...
import { CHUNK_TYPE, FRAME_TYPE, BUFFER_THRESHOLD, PLAYBACK_SPEED, AUDIO } from "./publisherConstants.js";
...
async function handleMediaWsMessage(event) {
  ...
  if (event.data instanceof ArrayBuffer) {
    ...
    let type;
    if (frameType === 0) type = CHUNK_TYPE.KEY;
    else if (frameType === 1) type = CHUNK_TYPE.DELTA;
    ...
    if (type === "audio") {
      ...
      audioDecoder.decode(chunk);
      return;
    } else if (type === CHUNK_TYPE.KEY || type === CHUNK_TYPE.DELTA) {
      // Video
      type === CHUNK_TYPE.KEY && (keyFrameReceived = true);
      if (keyFrameReceived) {
