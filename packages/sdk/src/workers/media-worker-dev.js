...
async function handleGopStream(stream, gopIndex) {
  ...
  try {
    ...
    while (true) {
      ...
      frameCount++;
      // Payload is the standard binary packet (PacketBuilder format):
      //   seq(4) + timestamp(4) + frameType(1) + data
      // Route through the existing pipeline used by the bidi stream.
      await processIncomingMessage(payload);
    }
